...
const total = ref(0)
const title = ref("")
const roomTypeOptions = ref([]) // 房型选项列表
...
async function getRoomTypes() {
  try {
    ...
    if (res.code === 200) {
      roomTypeOptions.value = res.rows || []
    } else {
      proxy.$modal.msgError("获取房型失败：" + res.msg)
    }
  } catch (error) {
    console.error("获取房型出错：", error)
    proxy.$modal.msgError("网络异常，请检查接口")
  }
}
